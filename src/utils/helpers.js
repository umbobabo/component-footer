...
export function createI13nProps(link, i13n, options) {
  return i13n ? { i13nModel: createI13nModel(link, i13n, options) } : {};
}
...
export function renderListOfLinks(
  listOfLinks,
  { useIcons = false, iconColor = "#B6B6B6" } = {},
  LinkComponent,
  i13n
) {
  ...
  return (
    ...
    listOfLinks.map((link, index) => {
      ...
      if (Array.isArray(link)) {
        // Override a specific LinkComponent.
      ...
      if (useIcons) {
        linkContents = (
          <Icon icon={link.meta} color={iconColor} size={iconSize} />
        );
      }
      const Link = createLinkTag(LinkRenderingComponent, i13n);
      const i13nProps = createI13nProps(link, i13n, {
        position: `${i13nPosition}.${index + 1}`
      });
...
export function renderNewsletterLink(social, LinkComponent, i13n) {
  ...
  const i13nProps = createI13nProps(newsletter, i13n, {
    position: i13nPosition
  });
  return (
    <Link
      className="ec-footer__link ec-footer__subscribe-newsletter-link"
      href={newsletter.href}
      {...targetIfNeeded(newsletter)}
      {...i13nProps}
    >
      <Icon
        icon="mail"
        className="ec-footer__subscribe-newsletter-icon"
        color="#B6B6B6"
        size={iconSize}
      />
      {newsletter.title}
